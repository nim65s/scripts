...
disk_password = getpass.getpass(
    prompt="Disk password, empty for no LUKS (won't echo): "
)
...
with open("/etc/pacman.d/mirrorlist.new") as in_f, open(
    "/etc/pacman.d/mirrorlist", "w"
) as out_f:
    for line in in_f:
...
def install(installation):
    if installation.minimal_installation():
        ...
        if netif:
            with open(
                f"{installation.mountpoint}/etc/systemd/network/20-wired.network", "w"
            ) as net:
                net.write(f"[Match]\nName={netif}\n\n[Network]\nDHCP=yes\n")
    ...
    if disk_password:
        ...
        with archinstall.luks2(
            ...
            with archinstall.Installer(
                unlocked_device, harddrive.partition[0], hostname=hostname
            ) as installation:
        ...
        with archinstall.Installer(
            harddrive.partition[1], harddrive.partition[0], hostname=hostname
        ) as installation:
            install(installation)

...
parser.add_argument(
    "--api_key",
    help="defaults to GANDI_API_KEY env var, or the return of 'pass api/gandi'",
)
...
if args.ip is None:
    args.ip = (
        requests.get("http://ifconfig.me", headers={"User-Agent": "curl/7.61.1"})
        .content.decode()
        .strip()
    )
...
if args.api_key is None:
    args.api_key = os.environ.get(
        "GANDI_API_KEY", check_output(["pass", "api/gandi"], text=True).strip()
    )

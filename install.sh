...
[[ -f /etc/arch-release ]]   && sudo pacman -Syu --noconfirm --needed git gvim fish openssh tinc python-pip rofi pass pcsc-tools ccid libusb-compat dunst msmtp-mta shellcheck dfc khal khard vdirsyncer todoman ncdu htop tig inetutils kitty iwd rustup docker-compose python-wheel python-i3ipc python-pandocfilters ipython fd ripgrep zellij just bacon sccache bat git-delta watchexec starship cargo-release
...
then
    ...
    [[ -f /etc/debian_version ]] && cargo binstall -y fd-find ripgrep zellij just bacon sccache bat git-delta watchexec-cli starship cargo-release
fi

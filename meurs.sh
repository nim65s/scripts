...
tous_les_tuer()
  {
    for ATUER in $(ps -ef | grep -v 'grep\|meurs' | grep $1 | sed 's/  */ /g' | cut --delimiter=" " -f 2)
      do
	kill $ATUER && echo processus $ATUER mort
      done
  }

NOMBRE=$(ps -ef | grep -v 'grep\|meurs' | grep $1 | sed 's/  */ /g' | cut --delimiter=" " -f 2 | wc -l)
case $NOMBRE in
  0)
    echo "pas de processus... fin du script";
    ;;
  1)
    kill $(ps -ef | grep -v 'grep\|meurs' | grep $1 | sed 's/  */ /g' | cut --delimiter=" " -f 2) && echo $1 est mort. RIP.
    ;;
  *)
...
case $NOMBRE in
  ...
  1)
    if [[ $defonce = 1 ]]
      then
	kill -9 $(ps -ef | grep -v 'grep\|meurs' | grep $1 | sed 's/  */ /g' | cut --delimiter=" " -f 2) && echo $1 est mort 9 fois. RIP.
      else

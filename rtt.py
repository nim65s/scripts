...
from json import (
    ...
    load,
    loads,
)  # noqa - flake8 doesn't find doctests
...
parser.add_argument(
    "-n", "--notify", action="store_true", help="send notifications if you are late"
)
parser.add_argument(
    "-p",
    "--path",
    type=Path,
    default=expanduser("~/.local/rtt.json"),
...
if __name__ == "__main__":
    ...
    if args.test:
        ...
        if hasattr(args, "name"):
            tasks.append(
                RecurrentTask(
                    args.name, " ".join(args.description), args.period, dt.now()
                )
            )
